...
C
      SUBROUTINE LMVY18 (M,N,Y,D,APNT,A)
C
C***********************************************************************
C
C     Created: 11 Jan 1994 (ACD)
C
C     Purpose: Perform a matrix-vector multiplication Y = Y + A*D*X,
C              assuming data structures used in sparse LDL' codes.
C              Uses level 8 loop unrolling.
C
C     Input:
C        M      - Number of rows in A
C        N      - Number of columns in A
C        Y      - Vector to which A*D*X will be added
C        D      - Correction factors for LDL', i.e. the diagonal
C        APNT   - Index vector for A
C                 APNT(I) points to the first nonzero in column I of A
C        A      - Nonzero terms of A
C
C     Output:
C        Y      - Contains Y = Y + A*D*X
C
C***********************************************************************
C
      IMPLICIT NONE
