...
C
      SUBROUTINE   SPRFC4   ( MSPAR , MTREES, MSIFA , SM    , TOL   ,
...
C ======================================================================
C  S A M  library routine :  SPRFC4                   GROUP 9 / PUBLIC
C ======================================================================
C
C     Purpose
C     -------
...
*                 Aug. 14, 2003 (kmo)
*                 Added MSPAR(57) in call to SPRF41.
C
C     MSPAR - INTEGER(*)
...
C     ------------------------------------------------------------------
     ...
     $  XSIBL , XSPLIT, XSUPER
      ...
      EXTERNAL          SPRF41, SPRER1, MVMULT, MMMULT

C     ==================================================================
#if FT_DEBUG > 1
      IF (LPU.GE.0) WRITE(LPU,*) 'ENTERING SPRFC4'
#endif
...
C     -------------------
      IF ( MSPAR(1) .LT. 0 ) THEN
         MSPAR(1) = -6
         IERR = -1
         CALL SPRER1 ( 30, 'SPRFC4', 0, 0, 0, LPU, IERR )
         RETURN
      ELSE
         MSPAR(1) = 6
         IERR = 0
...
C     ------------------------------------------
     ...
     $     RWUSED .GT. NRWORK                              ) THEN
         ...
         ENDIF
         IERR = -2
         CALL SPRER1 ( 36, 'SPRFC4', 0, 0, 0, LPU, IERR )
         RETURN
      ENDIF
...
C     ----------------------------------------------------
C     COMPUTE D AND C BY A CALL TO SPRF41 AND STORE IN SM.
C     ----------------------------------------------------
      CALL SPRF41 ( NSUPER, NEQ, NOFSUB, LFIRST, WSTACK, NZEROL,
     ...
     $              MVMULT, MMMULT )
      ...
      IF ( LIERR .NE. 0 ) THEN
         ...
         ELSE
            IERR = -4
         ENDIF
         CALL SPRER1 ( 40, 'SPRFC4', NUMNEG, SUPEQT, LIERR, LPU, IERR )
         IF ( LIERR .EQ. -33 ) THEN
            MSPAR(27) = -SUPEQT
...
C     ==================================================================
#if FT_DEBUG > 1
      IF (LPU.GE.0) WRITE(LPU,*) 'LEAVING  SPRFC4'
#endif
      RETURN

C     ------------------------------------------------------------------
C     END OF MODULE SPRFC4
      END


      SUBROUTINE   SPRF41   ( NSUPER, NEQ   , NOFSUB, LFIRST, WSTACK,
...
C                      THE SUPERNODE WITH FIRST EQUATION SUPEQT.
C              = -32 : PIVOT REDUCED TO .LT. ZERO IN EQUATION SUPEQT.
C              = -33 : PIVOT REDUCED TO ZERO IN SUPEQT EQUATIONS.
...
C     ------------------------------------------------------------------
     ...
     $                  SUPSZE, SUPVAR, UPDSZE
      ...
      EXTERNAL          SPRF12, SPRF13, SPRF14, SPRF45, SPRF16, SPRF18,
     $                  MVMULT, MMMULT
...
C           -----------------------------------------------------
            IF ( PANEL ) THEN
               ...
               J = XSPLIT(JS)
               CALL SPRF45 ( LENVAR, SUPVAR, PANEL, SPLIT(J),
     $                       STACK(IPJS), DIAG(FSTVAR), DCOR(1,FSTVAR),
     $                       TOL, ERROR, FIRST, MVMULT, MMMULT )

            ELSE

               CALL SPRF45 ( LENVAR, SUPVAR, PANEL, FIRST,
     $                       STACK(IPJS), DIAG(FSTVAR), DCOR(1,FSTVAR),
     $                       TOL, ERROR, FIRST, MVMULT, MMMULT )
...
C     ------------------------------------------------------------------
C     END OF MODULE SPRF41
      END


      SUBROUTINE   SPRF45   ( M     , N     , PANEL , SPLIT , X     ,
     $                        DIAG  , DCOR  , TOL   , ERROR , XPNT  ,
     $                        MVMULT, MMMULT                          )
...
C              < 0: PIVOT REDUCED TO .LT. ZERO IN COLUMN ABS(ERROR).
C
C     WORKING ARRAYS
C     --------------
C
...
C     SPRF47 : PARTIAL CC' FACTORIZATION.
C
C     INTRINSIC
C     ---------
C
...
C           ---------------------------------------------
            CALL SPRF47 ( MM, NN, XPNT(FSTCOL), X, M,
     $                    DIAG(FSTCOL), DCOR(1,FSTCOL),
     $                    TOL, ERROR, MVMULT )
...
C        --------------------------------------------
         CALL SPRF47 ( M, N, XPNT, X, M, DIAG, DCOR,
     $                 TOL, ERROR, MVMULT )
         IF ( ERROR .LT. 0 ) RETURN
         NXTCOL = FSTCOL + N
         Q      = M      - NXTCOL + 1
...
C     ------------------------------------------------------------------
C     END OF MODULE SPRF45
      END


      SUBROUTINE   SPRF47   ( M, N, XPNT, X, LDX, LZDIAG, DCOR, TOL,
     $                        IERR, MVMULT )
...
C                ERROR RETURN ONLY WHEN NEGETIVE PIVOT IS ENCOUNTERED.
C
...
C                 < 0 : EQUATION WHERE DIAGONAL ENTRY < 0 IS ENCOUNTERED
C
...
C        -----------------------------------
         ADIAG = LZDIAG(JCOL)
         DIAG  = X(JPNT)
         IF ( DIAG .LT. -TOL(1)*ABS(ADIAG) ) THEN
C           -------------------------------------------------
C           THE MATRIX IS NEGATIVE DEFINITE. CANNOT CONTINUE.
C           -------------------------------------------------
            IERR   = -JCOL
            TOL(2) = DIAG
            TOL(3) = ADIAG
            ...
            DCOR(2,JCOL) = DIAG
            RETURN
         ELSE IF ( DIAG .LE. TOL(1)*ABS(ADIAG) ) THEN
C           -------------------------------------------------
...
C     TODO: Insert DIAG = ONE instead and zero-out the whole column/row.
            DIAG = TOL(1)*MAX(ONE,ABS(TOL(4)*ADIAG))
            X(JPNT) = DIAG
            DCOR(2,JCOL) = DIAG
         ENDIF
