...
C
      SUBROUTINE   SPRBS1   ( MSPAR , MTREES, MSIFA , SM    , B     ,
...
C ======================================================================
C  S A M  library routine :  SPRBS1                   GROUP 9 / PUBLIC
C ======================================================================
C
C     Purpose
C     -------
...
C                 Replaced SPRBS5 by SPRBS4 and SPRBS5.
C                 Replaced SPRBS9 by SPRZR0 and SPRZR1.
C                 Aug. 14, 2003 (kmo)
C                 Added MSPAR(57) in calls to SPRBS4 and SPRBS5.
C                 Aug. 27, 2005 (kmo)
...
C      Exit  : Not changed, except for MSPAR(1) that is set to 7
C              in case of sucessful exit, and to -7 else.
...
C      Entry : The forward and diagonal solved right-hand sides
...
C     ------------------------------------------------------------------
     ...
     $  XLINDX, XSUPER
      ...
      EXTERNAL          SPRBS4, SPRBS5, SPRER1

C     ==================================================================

#if FT_DEBUG > 1
      IF (LPU.GE.0) WRITE(LPU,*) 'ENTERING SPRBS1'
#endif
...
C     --------------------
      IF ( MSPAR(1) .LT. 0 ) THEN
         MSPAR(1) = -7
         IERR = -1
         CALL SPRER1 ( 30, 'SPRBS1', 0, 0, 0, LPU, IERR )
         RETURN
      ELSE
         MSPAR(1) = 7
         IERR = 0
...
C     Solve for only the first LDB (internal) equations when LDB < NEQ
     ...
     $     RWUSED .GT. NRWORK .OR. LDB    .LT. NEQ         ) THEN
         ...
         ENDIF
         IERR = -2
         CALL SPRER1 ( 37, 'SPRBS1', 0, 0, 0, LPU, IERR )
         RETURN
      ENDIF
...
C        ----------------------------------------
C        BACKWARD SOLVE IRHS BY A CALL TO SPRBS5.
C        ----------------------------------------
         IF ( MSPAR(58).GE.1 ) THEN
            CALL SPRBS5 (NSUPER, NEQ, MFRONT, NOFSUB, NZEROL,
     $                   MSPAR(57), MTREES(XSUPER), MTREES(XLINDX),
     $                   MTREES(SUPSUP), MSIFA(LINDX), MSIFA(PERI2E),
     $                   B(1,IRHS), SM(LNZ), RWORK(STACK))
         ELSE
            CALL SPRBS4 (NSUPER, NEQ, MFRONT, NOFSUB, NZEROL,
     $                   MSPAR(57), MTREES(XSUPER), MTREES(XLINDX),
...
#if FT_DEBUG > 1
      IF (LPU.GE.0) WRITE(LPU,*) 'LEAVING  SPRBS1'
#endif
      RETURN

C     ------------------------------------------------------------------
C     END OF MODULE SPRBS1
      END


      SUBROUTINE   SPRBS4   ( NSUPER, NEQ   , MFRONT, NOFSUB,
...
C                 Renamed from SPRBS5 to SPRBS4.
C                 AUG. 14, 2003 (KMO)
...
C              ON ENTRY : FORWARD AND DIAGONAL SOLVED VECTOR.
...
C           -----------------------------------------------------
            ...
            CALL SPRBS6 ( SUPVAR, JSLEN, LNZ(IPLNZ), LOCRHS )
...
C     ------------------------------------------------------------------
C     END OF MODULE SPRBS4
      END


      SUBROUTINE   SPRBS5   ( NSUPER, NEQ   , MFRONT, NOFSUB,
     $                        NZEROL, LPAR57, XSUPER, XLINDX, SUPSUP,
...
C
C --- TO PERFORM AN IN-CORE MULTIFRONT BACKWARD SOLUTION STEP, THIS IS
C     THE TOP LEVEL DRIVER. Identical to SPRBS4 except for the internal
C     permutation array PERI2E.
C
C     CREATED   : JAN. 11, 1994 (ACD)
C     REVISIONS : FEB. 25, 2003 (KMO)
C                 Made a new SPRBS5 from SPRBS4 by adding PERI2E, and
...
C     ------------------------------------------------------------------
C     END OF MODULE SPRBS5
      END


      SUBROUTINE   SPRBS6   ( SUPVAR, LENVAR, LNZ   , LOCRHS )
