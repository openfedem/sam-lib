...
C
      SUBROUTINE LMMY18 (M,N,Q,XPNT,X,D,Y,LDY)
C
C***********************************************************************
C
C     Created: 11 Jan 1994 (ACD)
C
C     Purpose: Perform a matrix-matrix multiplication, Y = Y + X*A*D,
C              assuming data structures used in the sparse LDL' codes.
...
C        Y      - Matrix to which X*A*D will be added
C        LDY    - Length of first column of Y
C
C     Output:
C        Y      - Contains Y = Y + X*A*D
C
C***********************************************************************
C
      IMPLICIT NONE
...
C        Update index range variables
         ...
         M2  = IY1 + M1
C
C        Perform approprate matrix vector multiplication X*A(*,YCOL)*D
